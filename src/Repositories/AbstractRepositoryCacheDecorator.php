...
abstract class AbstractRepositoryCacheDecorator implements AbstractRepositoryContract, CacheableContract
{
    ...
    /**
     * @var \Edutalk\Base\Caching\Services\CacheService
     */
    protected $cache;
    ...
    public function __construct(CacheableContract $repository, $cacheKeyGroup = null)
    {
        ...
        $this->cache = app(\Edutalk\Base\Caching\Services\Contracts\CacheServiceContract::class);

        $this->cache
            ->setCacheObject($this->repository)
            ->setCacheGroup($cacheKeyGroup)
            ->setCacheLifetime(config('edutalk-caching.repository.lifetime'))
            ->setCacheFile(config('edutalk-caching.repository.store_keys'));
    }
